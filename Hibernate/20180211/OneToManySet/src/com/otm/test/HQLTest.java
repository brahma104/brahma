...
public class HQLTest {
	...
	private static void showFlatsByTypeSqftAndFacingC(Session session, String flatType, int startSize, int maxSize,
		...
		c = session.createCriteria(Flat.class).add(Restrictions.and(Restrictions.eq("flatType", flatType),
				Restrictions.and(Restrictions.between("sqft", startSize, maxSize), Restrictions.eq("facing", facing))));
		List<Flat> flats = c.list();
		for (Flat f : flats) {
			System.out.println(f);
		}
	...
	private static void showNoOfFlatsInBlocksC(Session session) {
		Criteria c = session.createCriteria(Flat.class).setProjection(Projections.projectionList()
				.add(Projections.groupProperty("block.blockNo")).add(Projections.count("flatNo")));
	...
	private static void showFlatsByFloor(Session session, int floor) {
		Query<com.otm.bo.Flat> query = null;
	...
	private static void showFlatsAndBlocksByFlatTypeSqftFacingAndUnitsC(Session session, String flatType, String facing,
			int sqft, int units) {
		Criteria c = session.createCriteria(Flat.class).createAlias("block", "b")
				.add(Restrictions.and(Restrictions.eq("flatType", flatType), Restrictions.eq("facing", facing),
